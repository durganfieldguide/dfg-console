...
// $10,000 bid should get 3% = $300, but capped at $150
{
  ...
  assert.strictEqual(
    acq.buyer_premium,
    150,
    'Tier 3: $10,000 bid should have $150 premium (3% capped)'
  )
...
// Test D: buildCalculationSpine also uses correct Sierra fees
{
  ...
  assert.strictEqual(
    spine.buyer_premium,
    75,
    'buildCalculationSpine: $1,600 bid should have $75 premium'
  )
...
// Test E: Sierra injection works (backward compatibility)
{
  ...
  assert.strictEqual(
    acq.buyer_premium,
    75,
    'Injected Sierra fees should yield $75 premium at $1,600 bid'
  )
...
// $1,000 bid with 0.15 (15%) premium → $150 (NOT $0.15)
{
  ...
  assert.strictEqual(
    acq.buyer_premium,
    150,
    '#126: 0.15 decimal → $150 premium on $1,000 bid (not $0.15)'
  )
  assert.strictEqual(acq.premium_method, 'percent', "Method should be 'percent' not 'flat'")
  console.log(
    `✓ #126 Decimal format: 0.15 on $1,000 bid → $${acq.buyer_premium} premium (expected $150)`
  )
}
...
// $1,000 bid with 15 (15%) premium → $150
{
  ...
  console.log(
    `✓ #126 Whole-number format: 15 on $1,000 bid → $${acq.buyer_premium} premium (expected $150)`
  )
}
...
// Test H: Edge case - 12% premium
{
  ...
  console.log(
    `✓ #126 Edge case: 0.12 on $5,000 bid → $${acq.buyer_premium} premium (expected $600)`
  )
}
...
// Verify the formula: margin = profit / acquisitionCost (NOT profit / salePrice)
{
  ...
  console.log(
    `✓ #127: $${profit} profit on $${acquisitionCost} acquisition → ${(spine.expected_margin * 100).toFixed(1)}% margin`
  )
...
// Test J: All three margin scenarios use correct denominator
{
  ...
  console.log(
    `  Quick=${(spine.quick_sale_margin * 100).toFixed(0)}%, Expected=${(spine.expected_margin * 100).toFixed(0)}%, Premium=${(spine.premium_margin * 100).toFixed(0)}%`
  )
}
