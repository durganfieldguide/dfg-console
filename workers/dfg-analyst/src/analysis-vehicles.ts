...
const VEHICLE_BASELINES: Record<
  string,
  { base: number; premium_mult: number; quick_mult: number }
> = {
  // Luxury SUVs - wider spreads due to variable buyer pool
  land_rover: { base: 12000, premium_mult: 1.35, quick_mult: 0.7 },
  range_rover: { base: 15000, premium_mult: 1.4, quick_mult: 0.65 },
  lexus: { base: 10000, premium_mult: 1.3, quick_mult: 0.75 },
  mercedes: { base: 11000, premium_mult: 1.35, quick_mult: 0.7 },
  bmw: { base: 9000, premium_mult: 1.3, quick_mult: 0.72 },
  audi: { base: 9000, premium_mult: 1.3, quick_mult: 0.72 },

  // Mainstream SUVs - tighter spreads due to consistent demand
  toyota: { base: 10000, premium_mult: 1.25, quick_mult: 0.8 },
  honda: { base: 9000, premium_mult: 1.25, quick_mult: 0.8 },
  ford: { base: 7000, premium_mult: 1.3, quick_mult: 0.75 },
  chevrolet: { base: 6500, premium_mult: 1.3, quick_mult: 0.75 },
  ...
  truck_ford: { base: 12000, premium_mult: 1.3, quick_mult: 0.78 },
  truck_chevrolet: { base: 11000, premium_mult: 1.3, quick_mult: 0.78 },
  truck_toyota: { base: 15000, premium_mult: 1.25, quick_mult: 0.82 },
  truck_ram: { base: 10000, premium_mult: 1.3, quick_mult: 0.75 },

  // Default fallback - wider spread for unknown vehicles
  default: { base: 5000, premium_mult: 1.35, quick_mult: 0.7 },
}
...
function getMileageMultiplier(mileage: number | null | undefined): number {
  ...
  if (mileage < 125000) return 0.9
  if (mileage < 150000) return 0.8
  if (mileage < 200000) return 0.65
  return 0.5 // 200k+ is budget territory
}
...
function getTitleMultiplier(titleStatus: string | undefined): number {
  ...
  switch (titleStatus.toLowerCase()) {
    ...
    case 'salvage':
      return 0.5
    case 'lien':
      return 0.9 // Assume lien will be cleared
    case 'branded':
      return 0.65
    default:
      return 0.85
...
function getConditionMultiplier(condition: VehicleCondition): number {
  ...
  if (condition.mechanical?.engine_status === 'cranks_no_start') mult *= 0.6
  if (condition.mechanical?.engine_status === 'no_crank') mult *= 0.4
  ...
  // Body damage
  if (condition.exterior?.body_damage === 'major_dents') mult *= 0.9
  if (condition.exterior?.body_damage === 'collision_damage') mult *= 0.7
...
export function lookupVehicleComps(
  ...
  if (
    ...
    modelLower.includes('1500') ||
    modelLower.includes('2500')
  ) {
...
export function calculateMinimumViableRepairVehicles(condition: VehicleCondition): RepairPlan {
  ...
  if (
    condition.mechanical?.brakes === 'needs_service' ||
    condition.mechanical?.brakes === 'grinding'
  ) {
  ...
  return {
    ...
    assumptions: [
      'DIY-friendly items only',
      'Phoenix metro pricing',
      'No major mechanical work (walk away instead)',
    ],
  }
}
