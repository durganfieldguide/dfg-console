 ...
 */
export function calculateConfidenceScore(condition: ConditionAssessment): number {
  let score = 5
...
export function evaluateGates(params: {
  ...
  gates.push({
    ...
    reason:
      condition.photos_analyzed != null
        ? `Only ${condition.photos_analyzed} photo(s) analyzed`
        : undefined,
  })
...
export function buildReportFields(params: {
  ...
  const maxBidBest = Math.round(maxBidMid * 1.1) // upside if inspection clean
...
export function buildReportMarkdown(params: {
  ...
  lines.push(
    `**Bid Range:** ${fmtMoney(Math.round(maxBid * 0.85))} – ${fmtMoney(Math.round(maxBid * 1.1))}`
  )

  if (acquisition?.total_acquisition != null) {
    lines.push(
      `**All-In Cost (est):** ${fmtMoney(acquisition.total_acquisition)} (bid + premium + tax)`
    )
  }

  if (scenarios?.expected) {
    lines.push(
      `**Expected Profit:** ${fmtMoney(scenarios.expected.gross_profit)} @ ${fmtPct(scenarios.expected.margin)} margin`
    )
  }
  if (scenarios?.quick_sale) {
    lines.push(
      `**Quick-Sale Floor (7d):** ${fmtMoney(scenarios.quick_sale.gross_profit)} @ ${fmtPct(scenarios.quick_sale.margin)}`
    )
  }

  lines.push(
    `**Confidence:** ${'★'.repeat(confidence)}${'☆'.repeat(5 - confidence)} (${confidence}/5)`
  )
  ...
  if (buyerLens?.perceived_value_range) {
    ...
    if (objections.length) {
      ...
      objections.slice(0, 3).forEach((o: any) => {
        lines.push(
          `• "${o.concern}" → Rebuttal: ${o.rebuttal_strategy || 'address in listing copy'}`
        )
      })
    }
  }
