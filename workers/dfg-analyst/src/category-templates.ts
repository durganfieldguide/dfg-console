 ...
 */
export function detectAssetType(categoryId: string | null | undefined, title: string | null | undefined): AssetType {
  const cat = (categoryId || '').toLowerCase();
  const titleLower = (title || '').toLowerCase();

  // Explicit vehicle categories
  if (cat === 'fleet_trucks' || cat === 'vehicles' || cat === 'vehicle' ||
      cat === 'cars_trucks' || cat === 'auto' || cat === 'automotive' ||
      cat.includes('vehicle') || cat.includes('car') || cat.includes('truck') || cat.includes('suv')) {
    return 'vehicle';
  }
  ...
  const vehicleMakes = [
    ...
    'crown victoria', 'tahoe', 'suburban', 'expedition', 'yukon', 'silverado',
    'sierra', 'f-150', 'f-250', 'f-350', 'mustang', 'camaro', 'corvette'
  ];

  const vehicleTypes = ['sedan', 'coupe', 'suv', 'crossover', 'hatchback', 'wagon',
                        'convertible', 'minivan', 'pickup', 'crew cab', 'extended cab'];

  const trailerKeywords = ['trailer', 'flatbed', 'enclosed', 'utility trailer', 'cargo trailer',
                           'dump trailer', 'car hauler', 'equipment trailer', 'tilt deck'];

  const powerToolKeywords = ['drill', 'saw', 'grinder', 'sander', 'router', 'planer',
                             'jointer', 'lathe', 'welder', 'compressor', 'generator',
                             'milwaukee', 'dewalt', 'makita', 'bosch', 'ryobi'];

  // Check title for asset type indicators
  if (vehicleMakes.some(make => titleLower.includes(make)) ||
 ...
 */
export function buildIfBiddingSteps(assetType: AssetType, maxBid: number, inspectionPriorities: string[]): string[] {
  ...
  steps.push('KILL ZONE: Enter your absolute max once. No incremental bidding. Walk away if outbid.');
 ...
 */
export function buildListingPrepSteps(assetType: AssetType, repairItems?: Array<{ marketing_note?: string }>): string[] {
  const steps: string[] = [];
