...
export function buildCalculationSpine(args: {
  bidAmount: number;
  feeSchedule: {
    buyer_premium: number | { tiers: Array<{ max_bid: number; premium: number }>; above_threshold_percent?: number };
...
export function evaluateConditionScore(condition: {
  ...
  const gradeToScore: Record<string, number> = {
    ...
    'functional': 4, 'needs_work': 2, 'unknown': 0, 'not_visible': 0
  };
...
export function buildUnifiedConfidence(args: {
  ...
  const coverageRatio = evidenceCoverage.total_claims > 0
