...
export function calculateMinimumViableRepairPowerTools(condition: any): RepairPlan {
  ...
  if (
    batteryCondition === 'dead' ||
    batteryCondition === 'swollen' ||
    batteryCondition === 'missing'
  ) {
    const voltage = condition.battery_system?.voltage || '20V'
    const costPerBattery =
      voltage === '60V' || voltage === '40V'
        ? 120
        : voltage === '20V' || voltage === '18V'
          ? 60
          : 40 // 12V
  ...
  if (
    condition.condition?.chuck_or_blade === 'worn' ||
    condition.condition?.chuck_or_blade === 'damaged'
  ) {
    items.push({
      item: 'Chuck/blade replacement',
      approach: 'Standard replacement',
      cost: 25,
      required: true,
    })
  }
  ...
  if (condition.condition?.cosmetic === 'heavy_wear') {
    items.push({
      item: 'Deep clean + detail',
      approach: 'DIY cleaning',
      cost: 0,
      required: false,
      marketing_note: 'Professionally cleaned',
    })
  }
...
export function lookupPowerToolComps(
  ...
  const voltageMultiplier =
    batteryVoltage === '60V' || batteryVoltage === '40V'
      ? 1.4
      : batteryVoltage === '20V' || batteryVoltage === '18V'
        ? 1.2
        : 1.0 // 12V or corded
