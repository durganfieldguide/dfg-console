...
export default {
	async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
        ...
        `;
				...
				const result = await env.DFG_DB.prepare(query)
					.bind(...params)
					.all();
				...
				const countResult = await env.DFG_DB.prepare(countQuery)
					.bind(...countParams)
					.first();
		...
		if (cleanPath.startsWith('/ops/analysis/') && request.method === 'GET') {
			...
			try {
				const result = await env.DFG_DB.prepare('SELECT * FROM analyses WHERE listing_id = ?').bind(listingId).first();
        ...
        `,
				...
				await env.DFG_DB.prepare('UPDATE listings SET status = ? WHERE id = ?').bind('analyzed', listingId).run();
				...
				for (const row of candidates.results || []) {
					...
					try {
						...
						if (adapter?.hydratePhotosForLot) {
							...
							if (photos.length > 0) {
								await env.DFG_DB.prepare(`UPDATE listings SET photos = ? WHERE id = ?`).bind(JSON.stringify(photos), r.id).run();
        ...
        `,
				...
				return json({
					...
					results,
					message:
						remainingCount > 0 ? `Run again to process next batch (${remainingCount} remaining)` : 'All candidates have been hydrated',
				});
			} catch (err: any) {
