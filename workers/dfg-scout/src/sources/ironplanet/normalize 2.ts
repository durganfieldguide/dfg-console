...
function parseTimeLeft(timeLeft: string | undefined, nowMs: number): { endAt: string | undefined; status: LotStatus } {
  ...
  if (monthMatch) {
    ...
    const status: LotStatus = endDate.getTime() < nowMs ? 'closed' :
                              endDate.getTime() - nowMs < 24 * 60 * 60 * 1000 ? 'closing' : 'active';
    return { endAt, status };
  }
...
export function normalizeIronPlanetLot(raw: IronPlanetQuickview, nowMs: number = Date.now()): NormalizedLot {
  ...
  const imageUrl = coerceNonEmptyString(raw.photoBigger)
    || coerceNonEmptyString(raw.photo)
    || coerceNonEmptyString(raw.photoThumb);
  ...
  // Features as tags
  const sourceTags = raw.features?.split('|').map(f => f.trim()).filter(Boolean);
