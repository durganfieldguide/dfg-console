import { NormalizedLot, NormalizedPrice, NormalizationResult, LotStatus, validateNormalizedLot } from '../../core/types';
 ...
 */
export function determineLotStatus(endTimeIso: string | null | undefined, nowMs: number = Date.now()): LotStatus {
...
export function normalizeSierraLot(raw: any, nowMs: number = Date.now()): NormalizedLot {
	...
	if (!auctionId || !lotId) {
		throw new Error(
			`Missing required identifiers: auction_id=${String(raw?.auction_id ?? '')}, auction_lot_id=${String(raw?.auction_lot_id ?? '')}`,
		);
	}
	...
	const imageUrl =
		coerceNonEmptyString(raw?.primary_image?.large) ||
