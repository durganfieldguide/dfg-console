...
export async function runScout(
  ...
  for (const sourceName of sourcesToRun) {
    ...
    const routerLoaded = await trackSubrequest(
      () => router.load(sourceName),
      subreq,
      SUBREQUEST_LIMIT,
      `Router Load (${sourceName})`
    );
    ...
    try {
      ...
      for (const auction of auctions) {
        ...
        for (const lot of lots) {
          ...
          const photosJson = lot.photoUrls && lot.photoUrls.length > 0
            ? JSON.stringify(lot.photoUrls)
            : null;
      ...
      const processCandidate = async (item: typeof candidatesToSync[0]) => {
        try {
          ...
          if (item.needsSnapshot) {
            ...
            await env.DFG_DB.prepare(`UPDATE listings SET r2_snapshot_key = ? WHERE id = ?`)
              .bind(snapshotKey, item.d1Id)
              .run();
