...
function toRouterInputFromFixture(lot: any): RouterInput {
	...
	const description = String(lot?.description ?? lot?.desc ?? lot?.lot_description ?? lot?.lotDescription ?? '').trim();
...
async function main() {
	...
	for (const f of files) {
		...
		if (typeof expCand === 'boolean' || typeof expCat === 'string') {
			console.log(`   exp: ${expCat ?? '(any)'} candidate=${typeof expCand === 'boolean' ? expCand : '(any)'}`);
		}
