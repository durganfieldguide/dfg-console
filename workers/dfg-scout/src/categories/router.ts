...
function includesKeyword(text: string, keyword: string): boolean {
	...
	const core = tokens.length > 1 ? tokens.map(escapeRegExp).join('[\\s-]+') : escapeRegExp(k);
 ...
 */
function evaluateHardGates(gates: HardGate[], input: RouterInput): Array<{ field: string; reason: string; action: 'reject' | 'flag' }> {
