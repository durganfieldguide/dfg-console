-- Migration 003: Remove last_synced_to_coda column (Coda integration removed)
-- Run: npx wrangler d1 execute dfg-scout-db --file=./migrations/003-drop-coda-column.sql --remote

-- SQLite doesn't support DROP COLUMN directly, so we need to recreate the table
-- This is a destructive migration that removes the last_synced_to_coda column

-- Step 1: Create new table without the coda column
...
-- Step 2: Copy data (excluding last_synced_to_coda)
INSERT INTO listings_new (
  id, source, source_id, url, title, description, current_bid, end_time, location,
  location_text, auction_end_at, image_url, photos,
  ...
  price_kind, price_verified, lot_status,
...
-- Step 5: Recreate all indexes (without the coda sync index)
CREATE UNIQUE INDEX IF NOT EXISTS idx_listings_source ON listings(source, source_id);
CREATE INDEX IF NOT EXISTS idx_listings_sync ON listings(status, sync_state, next_sync_after);
...
CREATE INDEX IF NOT EXISTS idx_attack_delta ON listings(price_delta) WHERE status='candidate' AND sub_status='attack';
