...
export function computeGates(
  ...
  const criticalOpen = gates.filter((g) => g.severity === 'CRITICAL' && g.status === 'open').length
 ...
 */
const DISQUALIFYING_TITLE_STATUSES: string[] = ['salvage', 'parts_only']

/**
 * Lien statuses that should trigger auto-rejection when operator-verified.
 */
const DISQUALIFYING_LIEN_STATUSES: string[] = ['lien_present']
 ...
 */
export function checkHardGateFailures(operatorInputs: OperatorInputs | null): HardGateFailure[] {
