...
async function processListing(env: Env, listing: IngestListing): Promise<ProcessOutcome> {
...
async function createOpportunity(env: Env, listing: IngestListing): Promise<void> {
  const id = generateId()
  const now = nowISO()
  ...
  const auctionEndsAt = listing.end_time ? new Date(listing.end_time * 1000).toISOString() : null
 ...
 */
export async function ingestFromScout(
  env: Env
): Promise<IngestResult & { photos_synced?: number }> {
  ...
  for (const row of opportunities.results || []) {
    ...
    try {
      ...
      `
        ...
        .bind(JSON.stringify(photos), photos[0] || null, now, r.opportunity_id)
        .run()
