...
export function parseConditionData(raw: any): ConditionArea[] {
  ...
  if (typeof raw === 'object') {
    const knownAreas = [
      { key: 'exterior', name: 'Exterior', icon: <SparklesIcon className="h-4 w-4" /> },
      { key: 'interior', name: 'Interior', icon: <EyeIcon className="h-4 w-4" /> },
      { key: 'mechanical', name: 'Mechanical', icon: <WrenchScrewdriverIcon className="h-4 w-4" /> },
      { key: 'tires', name: 'Tires', icon: <EyeIcon className="h-4 w-4" /> },
      { key: 'engine', name: 'Engine', icon: <WrenchScrewdriverIcon className="h-4 w-4" /> },
      { key: 'transmission', name: 'Transmission', icon: <WrenchScrewdriverIcon className="h-4 w-4" /> },
      { key: 'electrical', name: 'Electrical', icon: <WrenchScrewdriverIcon className="h-4 w-4" /> },
      { key: 'body', name: 'Body', icon: <SparklesIcon className="h-4 w-4" /> },
      { key: 'paint', name: 'Paint', icon: <SparklesIcon className="h-4 w-4" /> },
      { key: 'frame', name: 'Frame', icon: <WrenchScrewdriverIcon className="h-4 w-4" /> },
    ];
...
function EvidenceTypeIcon({ type, className }: { type: EvidenceType; className?: string }) {
  ...
  switch (type) {
    ...
    case 'vin_decode':
      return <FingerPrintIcon className={iconClass} />;
    case 'seller_stated':
...
function EvidenceBadge({ evidence }: { evidence: SubsystemEvidence }) {
  ...
  return (
    <div className="inline-block">
      ...
      >
        {evidence.verified ? '✓' : '⚠'}
        {hasPhoto && <PhotoIcon className="h-3 w-3" />}
        {hasText && <DocumentTextIcon className="h-3 w-3" />}
        {isInferredOnly && <LightBulbIcon className="h-3 w-3" />}
        ...
        >
          ...
          <div className="flex items-center gap-2 mb-2 pb-2 border-b" style={{ borderColor: 'var(--border)' }}>
            {evidence.verified ? (
              ...
              <ExclamationCircleIcon className="h-4 w-4 text-yellow-600 dark:text-yellow-400" />
            )}
...
export function ConditionAssessment({ overallCondition, areas, summary, className, evidenceLedger }: ConditionAssessmentProps) {
  ...
  // Penalized score = raw score * coverage ratio
  // This prevents showing 4.0/5 when only 1 of 5 areas is known
  const penalizedScore = rawAverageScore * coverageRatio;
