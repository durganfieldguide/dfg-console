...
export function TabbedAnalysis({
  analysis,
  currentBid,
  sourceUrl,
  className,
  ...
  const repairItems = parseRepairPlan(
    Array.isArray(analysis?.investor_lens?.repair_plan)
      ? analysis.investor_lens.repair_plan
          .map((r: any) => `${r.item || r.name} $${r.cost || 0}`)
          .join(', ')
      : ''
  )
      ...
      <Tabs defaultValue="report">
              ...
              <div>
                ...
                <p
                  className={cn(
                    ...
                    fields.expected_margin * 100 >= 25
                      ? 'text-green-600'
                      : fields.expected_margin * 100 >= 15
                        ? 'text-yellow-600'
                        : 'text-red-600'
                  )}
                >
                  {fields.expected_margin > 0
                    ? `${(fields.expected_margin * 100).toFixed(0)}%`
                    : 'N/A'}
                </p>
              </div>
            </div>
        ...
        <TabsContent value="condition" className="pt-4">
            ...
            conditionAreas.length === 0 && (
              <div className="mt-6 p-6 text-center border border-gray-200 rounded-lg bg-gray-50 dark:border-gray-700 dark:bg-gray-800">
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  No condition assessment available
                </p>
              </div>
            )}
          ...
          {analysis?.condition?.red_flags && analysis.condition.red_flags.length > 0 && (
            <div className="mt-6 space-y-4">
              {(() => {
                const coreRisks = analysis.condition.red_flags.filter(
                  (f) => f.riskCategory === 'core_risk'
                )
                const optionalIssues = analysis.condition.red_flags.filter(
                  (f) => f.riskCategory === 'optional'
                )
                            ...
                            >
                              ...
                              <div className="flex items-start gap-2 mt-2 p-2 rounded bg-red-100 dark:bg-red-900/30">
                                ...
                                <p className="text-xs text-red-800 dark:text-red-200 italic">
                                  {flag.buyerImpact ||
                                    'Impact assessment pending - requires manual review'}
                                </p>
                              </div>
                            </li>
                          ))}
                      ...
                      <div className="p-4 rounded-lg border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/20">
                        ...
                        <ul className="space-y-3">
                          {optionalIssues.map((flag, i) => (
                            <li
                              key={i}
                              className="pl-3 border-l-2 border-gray-300 dark:border-gray-700"
                            >
                              <div className="flex items-start gap-2 mb-1">
                                <span
                                  className={cn(
        ...
        <TabsContent value="investor" className="pt-4">
          <div className="space-y-6">
            ...
            {!repairItems?.length &&
              ...
              !analysis?.investor_lens?.verdict_reasoning && (
                <div className="p-6 text-center border border-gray-200 rounded-lg bg-gray-50 dark:border-gray-700 dark:bg-gray-800">
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    No investor analysis available
                  </p>
                </div>
              )}
            ...
            {/* Deal Killers */}
            {analysis?.investor_lens?.deal_killers &&
              analysis.investor_lens.deal_killers.length > 0 && (
            ...
            {analysis?.investor_lens?.inspection_priorities &&
              analysis.investor_lens.inspection_priorities.length > 0 && (
        ...
        <TabsContent value="report" className="pt-4">
          ...
          {analysis?.report_markdown ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-sm">
                Click the &ldquo;Analyze&rdquo; button above to generate a Dual Lens analysis with
                Max Bid and Inspection Priorities.
              </p>
            </div>
          )}
          ...
          {analysis?.metadata?.analysis_duration_ms && (
            <p className="text-xs text-muted-foreground text-center mt-4">
              ...
              {analysis.metadata?.total_tokens &&
                ` • ${analysis.metadata.total_tokens.toLocaleString()} tokens`}
            </p>
          )}
        </TabsContent>
      </Tabs>
