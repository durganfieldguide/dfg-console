...
import { Image, CheckCircle, XCircle, HelpCircle, AlertTriangle } from 'lucide-react'
...
export function PhotoPipelineMetrics({ metrics, className }: PhotoPipelineMetricsProps) {
  ...
  const { available, availability_known, selected, analyzed_ok, analyzed_failed } = metrics
  ...
  return (
    <div className={cn('p-3 rounded-lg border text-sm', bgColor, borderColor, className)}>
      {/* Header with icon */}
        ...
        {analyzed_failed > 0 && (
          <>
            ...
            <span className="flex items-center gap-1">
              ...
              <span className="text-red-600 dark:text-red-400 font-bold">{analyzed_failed}</span>
      ...
      <div className="mt-2 text-xs space-y-1">
        {hasLimitedPhotos && (
          <p className="text-orange-600 dark:text-orange-400">
            Seller only provided {available} photo{available !== 1 ? 's' : ''} (need 4+ for full
            assessment)
          </p>
        )}
        {hasUnknownCount && (
          <p className="text-gray-500">Photo count unknown (legacy data or missing field)</p>
        )}
        ...
        {analysisSuccess && !hasFetchFailures && !hasLimitedPhotos && !hasUnknownCount && (
          <p className="text-green-600 dark:text-green-400">Full photo coverage achieved</p>
        )}
      </div>
    </div>
  )
...
export function PhotoPipelineMetricsCompact({ metrics, className }: PhotoPipelineMetricsProps) {
  if (!metrics) {
    return <span className={cn('text-xs text-gray-400', className)}>No metrics</span>
  }
  ...
  return (
    <span className={cn('text-xs font-mono flex items-center gap-1', className)}>
      ...
      <span>{availability_known ? available : '?'} avail</span>
      <span className="text-gray-300">·</span>
      <span className={analyzed_ok >= 4 ? 'text-green-600' : 'text-yellow-600'}>{analyzed_ok}</span>
