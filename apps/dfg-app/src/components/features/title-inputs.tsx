    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <label className="text-sm font-medium flex items-center gap-2 text-gray-900 dark:text-white">
          {label}
          {isCleared && <CheckCircle className="h-4 w-4 text-green-500" />}
...
export function TitleInputs({
  ...
  const [vin, setVin] = useState<string>(initialInputs?.title?.vin?.value || '')
  ...
  const buildInputs = useCallback((): OperatorInputs => {
    ...
    titleStatus,
    titleStatusSource,
    titleStatusVerification,
    titleInHand,
    titleInHandSource,
    titleInHandVerification,
    lienStatus,
    lienStatusSource,
    lienStatusVerification,
    vin,
    vinSource,
    vinVerification,
    odometerMiles,
    ...
    <Card>
      ...
      <CardContent className="space-y-6 px-3 sm:px-4">
        ...
        <TitleField
          ...
          onSourceChange={(s) => {
            setTitleStatusSource(s)
            markDirty()
          }}
          onVerificationChange={(v) => {
            setTitleStatusVerification(v)
            markDirty()
          }}
        ...
        >
          <Select
            value={titleStatus}
            onChange={(e) => {
              setTitleStatus(e.target.value as TitleStatus | '')
              markDirty()
            }}
        ...
        <TitleField
          ...
          onSourceChange={(s) => {
            setTitleInHandSource(s)
            markDirty()
          }}
          onVerificationChange={(v) => {
            setTitleInHandVerification(v)
            markDirty()
          }}
        ...
        >
          <Select
            value={titleInHand}
            onChange={(e) => {
              setTitleInHand(e.target.value as TriState | '')
              markDirty()
            }}
        ...
        <TitleField
          ...
          onSourceChange={(s) => {
            setLienStatusSource(s)
            markDirty()
          }}
          onVerificationChange={(v) => {
            setLienStatusVerification(v)
            markDirty()
          }}
        ...
        >
          <Select
            value={lienStatus}
            onChange={(e) => {
              setLienStatus(e.target.value as LienStatus | '')
              markDirty()
            }}
