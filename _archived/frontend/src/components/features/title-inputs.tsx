    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <label className="text-sm font-medium flex items-center gap-2">
          {label}
          {isCleared && (
            <CheckCircleIcon className="h-4 w-4 text-green-500" title="Gate cleared" />
          )}
          {value !== undefined && !isCleared && (
            <ExclamationTriangleIcon className="h-4 w-4 text-yellow-500" title="Needs verification" />
          )}
        </label>
        {helpText && (
...
export function TitleInputs({
  ...
  const [titleStatusSource, setTitleStatusSource] = React.useState<InputSource>(
    initialInputs?.title?.titleStatus?.source || 'listing'
  );
  const [titleStatusVerification, setTitleStatusVerification] = React.useState<VerificationLevel>(
    initialInputs?.title?.titleStatus?.verificationLevel || 'unverified'
  );

  const [titleInHand, setTitleInHand] = React.useState<TriState | ''>(
    initialInputs?.title?.titleInHand?.value || ''
  );
  const [titleInHandSource, setTitleInHandSource] = React.useState<InputSource>(
    initialInputs?.title?.titleInHand?.source || 'listing'
  );
  const [titleInHandVerification, setTitleInHandVerification] = React.useState<VerificationLevel>(
    initialInputs?.title?.titleInHand?.verificationLevel || 'unverified'
  );

  const [lienStatus, setLienStatus] = React.useState<LienStatus | ''>(
    initialInputs?.title?.lienStatus?.value || ''
  );
  const [lienStatusSource, setLienStatusSource] = React.useState<InputSource>(
    initialInputs?.title?.lienStatus?.source || 'listing'
  );
  const [lienStatusVerification, setLienStatusVerification] = React.useState<VerificationLevel>(
  ...
  const [vinSource, setVinSource] = React.useState<InputSource>(
    initialInputs?.title?.vin?.source || 'listing'
  );
  const [vinVerification, setVinVerification] = React.useState<VerificationLevel>(
    initialInputs?.title?.vin?.verificationLevel || 'unverified'
  );

  const [odometerMiles, setOdometerMiles] = React.useState<string>(
    initialInputs?.title?.odometerMiles?.value?.toString() || ''
  );
  const [odometerSource, setOdometerSource] = React.useState<InputSource>(
    initialInputs?.title?.odometerMiles?.source || 'listing'
  );
  const [odometerVerification, setOdometerVerification] = React.useState<VerificationLevel>(
  ...
  const buildInputs = React.useCallback((): OperatorInputs => {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({
        type: 'success',
        title: 'Inputs saved',
        description: 'Title information has been updated',
      });
      onSaveSuccess?.();
    },
    onError: (error) => {
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          ...
          {isDirty && (
            <span className="text-xs font-normal px-2 py-1 rounded" style={{ backgroundColor: 'var(--warning)', color: 'white' }}>
              Unsaved changes
            </span>
          )}
        </CardTitle>
